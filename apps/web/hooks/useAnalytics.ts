...
export function useCreateAnalyticsEvent() {
  ...
  return useMutation({
    ...
    onSuccess: newEvent => {
      // Invalidate all analytics queries to refetch
      queryClient.invalidateQueries({ queryKey: analyticsKeys.all });

      // Update specific entity queries if applicable
      if (newEvent.campaignId) {
        queryClient.invalidateQueries({
          queryKey: analyticsKeys.byCampaign(newEvent.campaignId),
        });
      }
      if (newEvent.contactId) {
        queryClient.invalidateQueries({
          queryKey: analyticsKeys.byContact(newEvent.contactId),
        });
      }
      if (newEvent.leadId) {
        queryClient.invalidateQueries({
          queryKey: analyticsKeys.byLead(newEvent.leadId),
        });
      }
    },
  });
