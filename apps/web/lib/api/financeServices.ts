...
/**
 * Finance and production-planning API surface.
 *
 * Money arrives from the server already rounded and as a string, so it is kept
 * as a string here too — parsing it into a JS number to display it is how
 * rounding drift gets into a balance sheet.
 */
...
export type FinanceSide = {
  /** The currency the figures on this object are expressed in. */
  currencyCode: string;
  outstanding: string;
  overdue: string;
  overdueCount: number;
  ...
  /** Every currency with open invoices, so mixed books are never hidden. */
  currencies: string[];
...
export type FinanceDashboard = {
  ...
  /** Null when the two sides are in different currencies — nothing to subtract. */
