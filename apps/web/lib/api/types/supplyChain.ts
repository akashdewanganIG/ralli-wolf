/**
 * Types for the supply-chain modules.
 *
 * Quantities, costs and totals arrive from the API as strings, not numbers:
 * they are Postgres `numeric` columns handled with arbitrary-precision decimals
 * on the server, and putting them through a JavaScript `number` would quietly
 * lose precision on large or fractional values. Use `toNumber` from
 * `lib/utils/decimal` for display and comparison only.
 */
