...
} from "@/lib/api/dataTransferService";
import { toast } from "@/lib/toast";

/**
 * Import and export for one dataset.
 *
 * Every list in the product needs the same two things, so they live here once
 * rather than in thirty pages. A caller says which dataset it is showing and
 * gets the buttons, the dialogs, the format choice, the page range, the
 * template download and the per-row error report — all behaving identically
 * wherever they appear.
 *
 * Import only renders when the server says the dataset can take one. Ledger
 * data — stock movements, invoices, payments — is derived from other records
 * and has to be created through the flows that keep it consistent, so those
 * lists export but do not import, and the button simply is not there.
 */

type Props = {
  /** Registry key, e.g. "purchase-orders". */
  entity: string;
  /** Overrides the server's label in button text and file names. */
  label?: string;
  /** Called after a successful import so the page can refetch. */
  onImported?: () => void;
  /**
   * Suppress the Import control even for a dataset that accepts one.
   *
   * For callers that already run their own importer with rules the generic
   * one does not have — the lead importer, for instance, detects duplicates
   * against existing records and hands back a downloadable error report.
   * Two Import buttons side by side would be worse than one.
   */
  allowImport?: boolean;
  /** Render as small controls to sit in a panel header. */
  size?: "sm" | "default";
  className?: string;
};
...
export function DataTransfer({
  ...
  // The catalogue is small and identical for everyone, so it is fetched once
  // per page load and shared by every instance on it.
  React.useEffect(() => {
    let alive = true;
    loadCatalogue()
      .then(list => {
      ...
      .catch(() => {
        // A missing catalogue must not break the page it sits on; the buttons
...
/** One in-flight request shared by every DataTransfer on the page. */
