...
export class GoodsReceiptController {
   ...
   * Record what physically arrived. Nothing hits stock until the receipt is
   * posted, which is what lets QC sit in between.
   */
   ...
   * Turn the accepted quantities into stock, create the cost layers, raise
   * putaway work and advance the purchase order.
   */
  async post(req: Request, res: Response) {
    ...
    try {
      ...
      const result = await prisma.$transaction(
        ...
        // A large multi-line receipt touches stock, lots, movements, putaway
        // tasks and the purchase order, so give it room beyond the 5s default.
  ...
  /**
   * POST /api/goods-receipts/lines/:lineId/quality-check
   * Inspect a receipt line and decide how much of it becomes stock.
   */
  async createQualityCheck(req: Request, res: Response) {
    ...
    try {
      ...
      // A failure is something the buying team has to act on today, so it is
      // pushed rather than left to be discovered on a report.
      if (
  ...
  /** GET /api/goods-receipts/quality-checks */
