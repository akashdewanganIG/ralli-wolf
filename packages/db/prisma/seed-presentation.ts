...
async function main() {
  assertDestructiveDatabaseAllowed("The presentation seed");
  ...
  if (!users.length)
    throw new Error("No users found — cannot attribute records.");
  ...
  for (const [name, description] of [
    ...
    [
      "Spare Parts",
      "Service parts stocked for warranty and after-sales repair",
    ],
    [
      "Raw Materials",
      "Copper, steel and polymer inputs consumed in production",
    ],
  ...
  const specs: P[] = [
    {
      code: "RW-AG4-800",
      name: "Angle Grinder 100 mm 800 W",
      cat: "Power Tools",
    ...
    {
      code: "RW-AG5-1200",
      name: "Angle Grinder 125 mm 1200 W",
      cat: "Power Tools",
      itemType: "FINISHED_GOOD",
      cost: 3180,
      uom: "EA",
    ...
    {
      code: "RW-HD26-800",
      name: "Rotary Hammer 26 mm 800 W",
      cat: "Power Tools",
      itemType: "FINISHED_GOOD",
      cost: 5640,
      uom: "EA",
    ...
    {
      code: "RW-CS7-1400",
      name: "Circular Saw 185 mm 1400 W",
      cat: "Power Tools",
      itemType: "FINISHED_GOOD",
      cost: 4980,
      uom: "EA",
    ...
    {
      code: "RW-PS-710",
      name: "Polisher / Sander 180 mm 710 W",
      cat: "Power Tools",
      itemType: "FINISHED_GOOD",
      cost: 3860,
      uom: "EA",
    ...
    {
      code: "CMP-GEA-M8",
      name: "Spiral Bevel Gear Pair M8",
      cat: "Spare Parts",
      itemType: "COMPONENT",
      cost: 290,
      uom: "SET",
    ...
    {
      code: "RM-CU-WIRE",
      name: "Enamelled Copper Wire 0.45 mm",
      cat: "Raw Materials",
    ...
    {
      code: "CON-ADH-401",
      name: "Threadlocker Adhesive 401",
      cat: "Consumables",
      itemType: "CONSUMABLE",
      cost: 480,
      uom: "EA",
  ...
  for (const s of specs) {
    products[s.code] = await db.product.create({
      data: {
        ...
        standardCost: money(s.cost),
        hsnCode: s.hsn,
        weightKg: s.kg ? qty(s.kg) : null,
        isPurchasable: s.buy ?? false,
        isSellable: s.sell ?? false,
        isManufactured: s.mfg ?? false,
        isStockTracked: true,
      },
    });
  }
  ...
  const whSpecs = [
    ...
    {
      code: "MUM-FG",
      name: "Finished Goods Store — Bhiwandi",
      type: "WAREHOUSE",
      city: "Bhiwandi",
      state: "Maharashtra",
      pin: "421302",
  ...
  for (let w = 0; w < whSpecs.length; w++) {
    ...
    const zones = [
      ...
      ...(s.type === "PLANT"
        ? [{ code: "PRD", name: "Production Line", zoneType: "PRODUCTION" }]
        : []),
    ];
    for (const z of zones) {
      ...
      const count =
        z.zoneType === "STORAGE" ? 6 : z.zoneType === "PICKING" ? 4 : 2;
  ...
  const binIn = (wh: string, zone: string) =>
    bins[wh]!.find(b => b.code.startsWith(zone))!;
  console.log(
    `  ${whSpecs.length} warehouses, ${Object.values(bins).flat().length} bins`
  );
  ...
  const supSpecs = [
    {
      code: "SUP-00001",
      name: "Bharat Electricals & Windings",
      city: "Pune",
      state: "Maharashtra",
      lead: 12,
      days: 30,
      items: ["CMP-ARM-800", "CMP-STA-800", "RM-CU-WIRE"],
    },
    {
      code: "SUP-00002",
      name: "Precision Bearings India",
      city: "Rajkot",
      state: "Gujarat",
      lead: 9,
      days: 45,
      items: ["CMP-BRG-6001", "CMP-GEA-M8"],
    },
    {
      code: "SUP-00003",
    ...
    },
    {
      ...
      lead: 5,
      days: 15,
      items: ["PKG-BOX-AG4", "PKG-BOX-HD26"],
    },
    {
      code: "SUP-00006",
      name: "Deccan Electricals & Switchgear",
      city: "Hyderabad",
      state: "Telangana",
      lead: 10,
      days: 30,
      items: ["CMP-SW-10A", "CMP-CBL-2M"],
    },
    {
      code: "SUP-00007",
      name: "Sundaram Lubricants",
      city: "Chennai",
      state: "Tamil Nadu",
      lead: 8,
      days: 30,
      items: ["CON-GRS-EP2", "CON-ADH-401"],
    },
  ];
  ...
  for (let i = 0; i < supSpecs.length; i++) {
    ...
    const sup = await db.supplier.create({
      data: {
        ...
        status: "ACTIVE",
        email: `sales@${host}.co.in`,
        phone: `9${String(870000000 + i * 1111111).slice(0, 9)}`,
        website: `https://www.${host}.co.in`,
        gstNumber: `27AABC${1000 + i}K1Z${i}`,
        panNumber: `AABC${1000 + i}K`,
        addressLine1: `Unit ${4 + i}, MIDC Phase II`,
        city: s.city,
        state: s.state,
        postalCode: "411018",
        country: "India",
        currencyCode: "INR",
        paymentTerms: `${s.days} days net`,
        creditDays: s.days,
        incoterms: "FOB",
        leadTimeDays: s.lead,
        minOrderValue: money(25000),
        bankName: "HDFC Bank",
    ...
    await db.supplierContact.create({
      data: {
        ...
        phone: `9${String(910000000 + i * 1234321).slice(0, 9)}`,
        isPrimary: true,
      },
    });
    ...
    for (const code of s.items) {
      ...
      const sp = await db.supplierProduct.create({
        data: {
          ...
          unitPrice: money(Number(p.standardCost)),
          currencyCode: "INR",
          minOrderQuantity: qty(50),
          packSize: qty(10),
          leadTimeDays: s.lead,
          validFrom: ago(200),
          isPreferred: true,
      ...
      await db.supplierPriceTier.create({
        data: {
          ...
          minQuantity: qty(500),
          unitPrice: money(Number(p.standardCost) * 0.94),
        },
      });
    }
    ...
    for (let m = 3; m >= 1; m--) {
      ...
      await db.supplierPerformance.create({
        data: {
          supplierId: sup.id,
          periodStart: ago(m * 90),
          periodEnd: ago((m - 1) * 90 + 1),
          totalOrders: orders,
          totalOrderValue: money(180000 + m * 45000),
          receiptsCount: receipts,
          onTimeReceipts: onTime,
          lateReceipts: receipts - onTime,
          onTimeDeliveryRate: money((onTime / receipts) * 100),
          receivedQuantity: qty(2400 + m * 300),
          acceptedQuantity: qty(2350 + m * 300),
          rejectedQuantity: qty(50),
          qualityAcceptanceRate: money(97 + (i % 3)),
          averageLeadTimeDays: money(s.lead + (i % 3)),
          priceVariancePercent: money(((i % 5) - 2) * 0.8),
          fillRate: money(94 + (i % 6)),
          overallScore: money(88 + ((i + m) % 10)),
...
async function customers(ctx: any) {
  ...
  const accountSpecs: [string, string, string][] = [
    ...
    [
      "Godrej Interio Contracts",
      "Furniture Manufacturing",
      "godrejinterio-contracts.in",
    ],
  ...
  for (const [name, industry, domain] of accountSpecs) {
    accounts.push(
      await db.account.create({
        data: { name, industry, website: `https://www.${domain}` },
      })
    );
  }
  ...
  const leadCompanies = [
    "Sterling Engineering Works",
    "Vertex Infra Projects",
    "Anand Auto Components",
    "Shree Ganesh Fabricators",
    "Precision Die Casting Co",
    "Metro Rail Contractors",
    "Coastal Shipyard Services",
    "Prime Modular Interiors",
    "Sunrise Solar EPC",
    "Deccan Tool Room",
    "Nagpur Heavy Machining",
    "Surat Textile Engineering",
  ];
      ...
      await db.lead.create({
        data: {
          ...
          lastName:
            i < PEOPLE.length
              ? l
              : `${l} ${String.fromCharCode(65 + (i % 26))}`,
          email: `${slug(f, l)}${i}@${company
            .toLowerCase()
            .replace(/[^a-z]/g, "")
            .slice(0, 14)}.in`,
          ...
          score: 40 + ((i * 7) % 55),
          qualityScore: 50 + ((i * 5) % 45),
          completenessScore: 60 + ((i * 3) % 35),
          countryCode: "91",
          city: cities[i % 6],
          state: states[i % 6],
          pincode: pins[i % 6],
  ...
  console.log(
    `  ${accounts.length} accounts, ${contacts.length} contacts, ${leads.length} leads`
  );
...
async function pipeline(ctx: any) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const opp = await db.opportunity.create({
      data: {
        ...
        createdBy: admin.id,
        stage,
        type: i % 3 === 0 ? "NEW_CUSTOMER" : "EXISTING_CUSTOMER_UPGRADE",
        status:
          stage === "CLOSED_WON"
            ? "APPROVED"
            : stage === "CLOSED_LOST"
        ...
        leadSource: "MANUAL",
        nextStep: stage.startsWith("CLOSED")
          ? null
          : "Site visit and trial demonstration",
        lossReason:
          stage === "CLOSED_LOST" ? "Lost on price to an imported brand" : null,
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const q = await db.quote.create({
      data: {
        ...
        deliveryTerms: "Ex-works Bhiwandi",
        ...
        presentedAt:
          status === "PRESENTED" || status === "ACCEPTED" ? ago(6) : null,
    ...
    for (let j = 0; j < 3; j++) {
      ...
      await db.quoteLineItem.create({
        data: {
          ...
          listPrice: money(list),
          unitPrice: money(unit),
          totalPrice: money(unit * n),
          discount: money(2 + j * 2),
          sortOrder: j + 1,
        },
      });
    }
    await db.quote.update({
      ...
      data: {
        ...
        taxPercent: money(18),
        taxAmount: money(subtotal * 0.18),
        shippingAmount: money(2500),
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const so = await db.salesOrder.create({
      data: {
        ...
        ownerId: pick(i).id,
        status,
        orderDate: ago(24 - i * 2),
        expectedShipDate: days(4 + i),
        expectedDeliveryDate: days(8 + i),
        actualShipDate: ["SHIPPED", "DELIVERED"].includes(status)
          ? ago(3)
          : null,
        actualDeliveryDate: status === "DELIVERED" ? ago(1) : null,
        approvedAt:
          status === "DRAFT" || status === "PENDING_APPROVAL" ? null : ago(10),
        approvedById:
          status === "DRAFT" || status === "PENDING_APPROVAL" ? null : admin.id,
        paymentTerms: "30 days from invoice",
        deliveryTerms: "Ex-works Bhiwandi",
        shippingCity: "Mumbai",
  ...
  for (let i = 0; i < 6; i++) {
    ...
    await db.approvalProcess.create({
      data: {
        ...
        targetRecordId: quotes[i % quotes.length]!.id,
        requestedToId: admin.id,
        createdById: pick(i + 1).id,
        status: state,
        comment:
          state === "REJECTED"
            ? "Discount exceeds the agreed band for this account."
            : null,
  ...
  console.log(
    `  3 price books, ${opportunities.length} opportunities, ${quotes.length} quotes, 8 orders`
  );
...
async function inventory(ctx: any) {
    ...
    for (const whCode of ["MUM-PLANT", "MUM-FG"]) {
      ...
      const lot = await db.stockLot.create({
        data: {
          ...
          originalQuantity: qty(n),
          remainingQuantity: qty(n),
          unitCost: money(Number(p.standardCost)),
          sourceType: "PURCHASE",
          sourceReference: `GRN-2026-${String(700 + (lotNo % 5)).padStart(5, "0")}`,
          status: "ACTIVE",
          receivedAt: ago(40 - (lotNo % 30)),
          manufacturedDate: ago(60 - (lotNo % 30)),
          expiryDate: p.shelfLifeDays
            ? days(p.shelfLifeDays - 400 + (lotNo % 60))
            : null,
        },
      });
      lots.push(lot);
      ...
      await db.stockBalance.create({
        data: {
          ...
          lotId: lot.id,
          quantity: qty(n),
          reservedQuantity: qty(n * 0.12),
          status: "AVAILABLE",
          lastMovementAt: ago(6),
        },
      });
      await db.stockMovement.create({
        data: {
          ...
          referenceNumber: `GRN-2026-${String(700 + (mov % 5)).padStart(5, "0")}`,
          performedById: admin.id,
          notes: "Goods receipt posted to stock.",
  ...
  for (let i = 0; i < 40; i++) {
    ...
    await db.stockMovement.create({
      data: {
        ...
        lotId: lot.id,
        ...(direction === "IN"
          ? { toWarehouseId: lot.originWarehouseId }
          : { fromWarehouseId: lot.originWarehouseId }),
        quantity: qty(n),
        unitCost: money(Number(lot.unitCost)),
        totalCost: money(n * Number(lot.unitCost)),
        reasonCode: type === "SCRAP" ? "DAMAGED_IN_HANDLING" : null,
        performedById: pick(i).id,
        occurredAt: ago(28 - (i % 28)),
      },
    });
  }
  ...
  for (let i = 0; i < 6; i++) {
    ...
    await db.stockReservation.create({
      data: {
        productId: lot.productId,
        warehouseId: lot.originWarehouseId,
        lotId: lot.id,
        quantity: qty(12 + i * 4),
        releasedQuantity: qty(0),
        status: "ACTIVE",
        referenceType: i % 2 === 0 ? "SALES_ORDER" : "PRODUCTION_ORDER",
        referenceId: salesOrders[i % Math.max(salesOrders.length, 1)]?.id ?? 1,
        referenceNumber:
          salesOrders[i % Math.max(salesOrders.length, 1)]?.orderNumber ??
          "SO-2026-03001",
        expiresAt: days(14),
        createdById: pick(i).id,
      },
    });
  }
  ...
  for (let i = 0; i < all.length; i++) {
    ...
    await db.reorderRule.create({
      data: {
        ...
        reorderPoint: qty(safety * 1.5),
        reorderQuantity: qty(safety * 3),
        maximumStock: qty(safety * 6),
        leadTimeDays: 7 + (i % 10),
        autoRequisition: i % 3 === 0,
        isActive: true,
    ...
    if (i % 3 === 0) {
      ...
      await db.stockAlert.create({
        data: {
          productId: p.id,
          warehouseId: wh.id,
          alertType: alertTypes[t],
          severity: severities[t],
          status: i % 6 === 0 ? "ACKNOWLEDGED" : "OPEN",
          currentQuantity: qty(safety * 0.6),
          thresholdQuantity: qty(safety * 1.5),
  ...
  const count = await db.stockCount.create({
    data: {
      countNumber: "CNT-2026-00014",
      warehouseId: warehouses["MUM-FG"].id,
      countType: "CYCLE",
      status: "IN_PROGRESS",
      scheduledDate: ago(2),
      startedAt: ago(1),
      countedById: pick(2).id,
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const system = 60 + i * 9;
    const counted =
      i % 4 === 0 ? system - 3 : i % 5 === 0 ? system + 2 : system;
    await db.stockCountLine.create({
      data: {
        stockCountId: count.id,
        productId: lot.productId,
        binId: binIn("MUM-FG", "STO").id,
        lotId: lot.id,
        ...
        varianceValue:
          i < 7 ? money((counted - system) * Number(lot.unitCost)) : money(0),
    ...
    if (i === 3) {
      const pkg = await db.package.create({
        data: {
          ...
          pickListId: pl.id,
          status: "PACKED",
          grossWeightKg: qty(18.4),
          lengthCm: qty(60),
          widthCm: qty(40),
          heightCm: qty(35),
          trackingNumber: "BLR9284471IN",
          carrier: "Blue Dart",
          packedById: admin.id,
          packedAt: ago(1),
        },
      ...
      for (const t of tasks) {
        await db.packageLine.create({
          data: {
            ...
            lotId: t.lotId,
            quantity: t.requestedQuantity.toString(),
          },
        });
      }
    }
  ...
  console.log(
    `  ${lots.length} lots, ${mov - 1} movements, alerts, counts and tasks`
  );
...
async function production(ctx: any) {
  ...
  for (let i = 0; i < buildable.length; i++) {
    ...
    const bom = await db.billOfMaterials.create({
      data: {
        ...
        productId: fg.id,
        name: `${fg.name} — production structure`,
        version: 1,
        revision: draft ? "B" : "A",
        status: draft ? "DRAFT" : "ACTIVE",
        isDefault: !draft,
        outputQuantity: qty(1),
        uomId: fg.uomId,
        laborCost: money(180 + i * 40),
        overheadCost: money(120 + i * 25),
        rolledUpCost: money(Number(fg.standardCost) * 0.72),
        costedAt: ago(20),
        effectiveFrom: ago(90),
    ...
    for (let j = 0; j < spec.parts.length; j++) {
      ...
      const comp = await db.bomComponent.create({
        data: {
          bomId: bom.id,
          componentProductId: products[code].id,
          lineNumber: j + 1,
          quantity: qty(n),
          uomId: products[code].uomId,
          scrapPercent: money(j % 3),
          operationSequence: (j + 1) * 10,
      ...
      if (code === "CMP-BRG-6001") {
        await db.bomComponentSubstitute.create({
          data: {
            ...
            substituteProductId: products["CMP-GEA-M8"].id,
            priority: 1,
            conversionFactor: qty(1),
            isActive: true,
    ...
    await db.bomChangeLog.create({
      data: {
        ...
        fieldName: "status",
        oldValue: "DRAFT",
        newValue: draft ? "DRAFT" : "ACTIVE",
        description: draft
          ? "Created as a draft revision."
          : "Activated after engineering sign-off.",
  ...
  for (let i = 0; i < poStatuses.length; i++) {
    ...
    const order = await db.productionOrder.create({
      data: {
        ...
        status,
        plannedStartDate: ago(Math.max(12 - i, 1)),
        plannedEndDate: days(4 + i),
  ...
  console.log(
    `  ${boms.length} bills of materials, ${poStatuses.length} production orders`
  );
...
async function purchasing(ctx: any) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const pr = await db.purchaseRequisition.create({
      data: {
        ...
        approvedById: ["APPROVED", "CONVERTED", "PARTIALLY_CONVERTED"].includes(
          state
        )
          ? admin.id
          : null,
        approvedAt: ["APPROVED", "CONVERTED", "PARTIALLY_CONVERTED"].includes(
          state
        )
          ? ago(4)
          : null,
  ...
  for (let i = 0; i < 9; i++) {
    ...
    const po = await db.purchaseOrder.create({
      data: {
        ...
        notes:
          "Deliver against the agreed schedule. Test certificates required.",
    ...
    const supProducts = await db.supplierProduct.findMany({
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const grn = await db.goodsReceiptNote.create({
      data: {
        ...
        receivedById: pick(i).id,
        status,
        receivedDate: ago(9 - i),
        supplierInvoiceNumber: `INV-${8800 + i}`,
        supplierInvoiceDate: ago(10 - i),
    ...
    for (let j = 0; j < poLines.length; j++) {
      ...
      const grnLine = await db.goodsReceiptLine.create({
        data: {
          ...
          lineNumber: j + 1,
          receivedQuantity: qty(received),
          acceptedQuantity: qty(accepted),
          rejectedQuantity: qty(rejected),
          unitCost: money(Number(line.unitPrice)),
          batchNumber: `B${3300 + i * 10 + j}`,
          manufacturedDate: ago(30),
          qcResult: j === 0 ? (i === 1 ? "FAIL" : "PASS") : "PENDING",
          rejectionReason: rejected
            ? "Winding resistance outside the specified band"
            : null,
      ...
      if (j === 0) {
        const qc = await db.qualityCheck.create({
          data: {
            ...
            sampleSize: qty(20),
            inspectedQuantity: qty(received),
            acceptedQuantity: qty(accepted),
            rejectedQuantity: qty(rejected),
            result: i === 1 ? "FAIL" : i === 2 ? "CONDITIONAL_PASS" : "PASS",
            defectType: i === 1 ? "ELECTRICAL" : null,
            inspectedById: pick(i + 1).id,
            inspectedAt: ago(8 - i),
            remarks:
              i === 1
                ? "Winding resistance outside the specified band."
                : "Sampled per plan. Conforms.",
          },
        });
    ...
    await db.goodsReceiptNote.update({
      where: { id: grn.id },
      data: {
        totalReceivedQuantity: qty(totalRecv),
        totalAcceptedQuantity: qty(totalAcc),
        totalRejectedQuantity: qty(totalRej),
        totalValue: money(totalVal),
      },
    });
  }
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const mr = await db.materialRequisition.create({
      data: {
        ...
        warehouseId: warehouses["MUM-PLANT"].id,
        productionOrderId:
          prodOrders[i % Math.max(prodOrders.length, 1)]?.id ?? null,
        requestedById: pick(i).id,
        status: state,
        requiredByDate: days(3 + i),
        purpose: "Issue to the assembly line against the current build order.",
        notes: "Collect from the plant stores counter.",
        issuedById:
          state === "ISSUED" || state === "PARTIALLY_ISSUED" ? admin.id : null,
        issuedAt:
          state === "ISSUED" || state === "PARTIALLY_ISSUED" ? ago(2) : null,
        createdAt: ago(7 - i),
      },
    });
  ...
  for (const code of [
    ...
    for (const rule of rules) {
      ...
      const openLines = await db.purchaseOrderLine.findMany({
        where: {
          ...
          purchaseOrder: {
            ...
            status: { notIn: ["CANCELLED", "DRAFT", "CLOSED"] },
          },
        },
      ...
      const incoming = openLines.reduce(
        (a: number, l: any) =>
          a + Math.max(Number(l.quantity) - Number(l.receivedQuantity), 0),
        0
      );
  ...
  console.log(
    "  6 requisitions, 9 purchase orders, 5 receipts, 5 material requisitions, 4 shaped shortages"
  );
...
async function marketing(ctx: any) {
  ...
  for (const [name, description] of [
    [
      "Maintenance buyers — West",
      "Contacts at plants across Maharashtra and Gujarat",
    ],
  ...
  const campaigns: [string, string, number, string][] = [
    [
      "Monsoon Service Drive 2026",
      "EMAIL",
      20,
      "Service reminder to maintenance buyers",
    ],
    [
      "New AG5 Launch Announcement",
      "WHATSAPP",
      12,
      "Launch of the 125 mm 1200 W angle grinder",
    ],
    [
      "Distributor Scheme — Q3",
      "EMAIL",
      6,
      "Volume scheme for channel partners",
    ],
  ...
  for (let i = 0; i < campaigns.length; i++) {
    ...
    await db.campaignChannel.create({
      data: {
        campaignId: c.id,
        channelType: channel,
        externalId: `presentation-campaign:${c.id}`,
      },
    });
    ...
    await db.whatsAppTemplate.create({
      data: {
        ...
        components: [
          {
            type: "BODY",
            text: "Hello {{1}}, this is Ralli Wolf regarding {{2}}. Reply STOP to opt out.",
          },
  ...
  for (let i = 0; i < 4; i++) {
    ...
    await db.optOut.create({
      data: {
        ...
        channel: "WHATSAPP",
        source: "KEYWORD",
        reason: "Replied STOP to a campaign message",
        optedOutAt: ago(12 - i),
      },
    });
  }
  ...
  for (const name of ["price", "dealer", "service", "warranty", "spares"]) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      await db.leadKeyword
        .create({ data: { leadId: lead.id, keywordId: k.id } })
        .catch(() => {});
    }
  }
  ...
  const notices: [string, string, string, string][] = [
    [
      "APPROVAL_REQUESTED",
      "Approval Requested — Quote QT-2026-02003",
      "Nisha Dudi submitted a quotation for your approval.",
      "/sales/approvals",
    ],
    [
      "STOCK_ALERT",
      "Inventory alerts",
      "4 critical stock alert(s) need attention",
      "/inventory/alerts",
    ],
    [
      "QC_FAILED",
      "Quality check failed — GRN-2026-00702",
      "Armature Assembly 800 W from Bharat Electricals & Windings: 10 rejected of 250 inspected.",
      "/purchasing/goods-receipts",
    ],
    [
      "PURCHASE_ORDER_APPROVED",
      "Purchase Order PO-2026-00603 approved",
  ...
  await db.globalSetting.upsert({
    ...
    create: {
      ...
      value: "Ralli Wolf presentation dataset",
...
async function planning(ctx) {
  ...
  for (const c of centreSpecs) {
    centres[c.code] = await db.workCenter.create({
      data: {
        code: c.code,
        name: c.name,
        warehouseId: plant.id,
        type: c.type,
        description: `${c.name} at ${plant.code}`,
        capacityMinutesPerDay: c.cap,
        efficiencyPercent: c.eff.toFixed(2),
        costPerHour: c.cost.toFixed(2),
        parallelCapacity: c.par,
        isActive: true,
      },
    });
  }
  ...
  for (const bom of boms) {
    for (let i = 0; i < ROUTING.length; i++) {
      ...
      await db.bomOperation.create({
        data: {
          bomId: bom.id,
          workCenterId: centres[wc].id,
          sequence: (i + 1) * 10,
          name,
          setupMinutes: setup,
          runMinutesPerUnit: run.toFixed(4),
          isBlocking: true,
        },
      });
    }
  ...
  for (const order of orders) {
    ...
    for (const op of routing) {
      const minutes = Math.ceil(
        op.setupMinutes + Number(op.runMinutesPerUnit) * qty
      );
  ...
  console.log(
    `  ${centreSpecs.length} work centres, routing on ${boms.length} BOMs, ${scheduled} orders scheduled`
  );
}
...
async function finance(ctx) {
  ...
  const purchaseOrders = await db.purchaseOrder.findMany({
    where: {
      status: {
        in: ["PARTIALLY_RECEIVED", "RECEIVED", "ACKNOWLEDGED", "SENT"],
      },
    },
  ...
  const salesOrders = await db.salesOrder.findMany({
    where: {
      status: { in: ["SHIPPED", "DELIVERED", "IN_FULFILLMENT", "APPROVED"] },
    },
    orderBy: { id: "asc" },
  });
    ...
    supplierInvoices.push(
      await db.supplierInvoice.create({
        data: {
          ...
          invoiceDate: days(apOffsets[i] - 30),
          dueDate: days(apOffsets[i]),
    ...
    customerInvoices.push(
      await db.customerInvoice.create({
        data: {
          ...
          salesOrderId: so.id,
          status: "APPROVED",
          invoiceDate: days(arOffsets[i] - 30),
          dueDate: days(arOffsets[i]),
          subtotal: sub.toFixed(2),
          taxAmount: tax.toFixed(2),
          totalAmount: (sub + tax).toFixed(2),
  ...
  const settle = async (invoice, side, fraction, method, offset) => {
    ...
    const delegate =
      side === "SUPPLIER" ? db.supplierInvoice : db.customerInvoice;
    ...
    await delegate.update({
      ...
      data: {
        ...
        status:
          paid >= total ? "PAID" : paid > 0 ? "PARTIALLY_PAID" : invoice.status,
      },
    });
  };

  if (supplierInvoices[0])
    await settle(supplierInvoices[0], "SUPPLIER", 0.45, "BANK_TRANSFER", -18);
  if (supplierInvoices[1])
    await settle(supplierInvoices[1], "SUPPLIER", 0.6, "CHEQUE", -11);
  if (supplierInvoices[4])
    await settle(supplierInvoices[4], "SUPPLIER", 1, "BANK_TRANSFER", -3);
  if (customerInvoices[0])
    await settle(customerInvoices[0], "CUSTOMER", 0.5, "UPI", -14);
  if (customerInvoices[1])
    await settle(customerInvoices[1], "CUSTOMER", 0.35, "BANK_TRANSFER", -7);
  if (customerInvoices[3])
    await settle(customerInvoices[3], "CUSTOMER", 1, "BANK_TRANSFER", -2);
...
main()
  .then(async () => {
    ...
    console.log(
      "\nlargest tables:",
      counts.map(c => `${c.table}=${c.rows}`).join("  ")
    );
    console.log("done.");
